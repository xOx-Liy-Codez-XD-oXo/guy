...
int main(int argc,char **argv) {
	...
	while(1) {									 // # # # # Main loop # # # 
		...
		if(comic) {
			...
			guPerspective(perspective, camFov * 1.15, comic > 115 ? 2.0f : 1.0f, 0.1F, 300.0F);
			GX_LoadProjectionMtx(perspective, GX_PERSPECTIVE);
			if(comic > 115) {
		...
		if(comic) {
			...
			//use dof memory for comic fraems
			if(!comiclear) {
				GX_SetTexCopySrc(256, 0, 256, 256);//off to the right
				GX_SetTexCopyDst(256, 256, GX_TF_RGBA8, GX_FALSE);
				GX_CopyTex(DofDefClr, GX_TRUE); //color that area off to the right purple. texture is rewritten immediatly
				GX_PixModeSync(); //sprinkling these innn right now..
				GX_CopyTex(DofDefClr + 262144, GX_FALSE); //copy the purple to the tex
				GX_CopyTex(DofDefClr + 524288, GX_FALSE); 
				GX_CopyTex(DofDefClr + 786432, GX_FALSE); 
				GX_PixModeSync();
				comiclear = 1;
				GX_SetCopyClear((GXColor){0x7f, 0x4d, 0x90, 255}, 0x00ffffff); //light pink
				GX_CopyTex(DofDefClr, GX_TRUE); //fix color of area used for copy
				GX_PixModeSync();
			}
			GX_SetCopyClear((GXColor){0x7f, 0x4d, 0x90, 255}, 0x00ffffff);
			GXTexObj comicATex, comicBTex, comicCTex, comicDTex, comicETex; //Reuse A+B memory for E
			GX_InitTexObj(&comicATex, DofDefClr, 256, 256, GX_TF_RGBA8, GX_CLAMP, GX_CLAMP, GX_FALSE);
			GX_InitTexObj(&comicBTex, DofDefClr + 262144, 256, 256, GX_TF_RGBA8, GX_CLAMP, GX_CLAMP, GX_FALSE);
			GX_InitTexObj(&comicCTex, DofDefClr + 524288, 256, 256, GX_TF_RGBA8, GX_CLAMP, GX_CLAMP, GX_FALSE);
			GX_InitTexObj(&comicDTex, DofDefClr + 786432, 256, 256, GX_TF_RGBA8, GX_CLAMP, GX_CLAMP, GX_FALSE);
			GX_InitTexObj(&comicETex, DofDefClr, 512, 256, GX_TF_RGBA8, GX_CLAMP, GX_CLAMP, GX_FALSE);
			int whichtoupdate = 0;
			...
			switch(whichtoupdate) {
				...
				case 1:
					GX_SetTexCopySrc(0, 0, 256, 256);
					GX_SetTexCopyDst(256, 256, GX_TF_RGBA8, GX_FALSE);
					GX_CopyTex(DofDefClr + 262144, GX_TRUE); 
					break;
				case 2:
					GX_SetTexCopySrc(0, 0, 256, 256);
					GX_SetTexCopyDst(256, 256, GX_TF_RGBA8, GX_FALSE);
					GX_CopyTex(DofDefClr + 524288, GX_TRUE); 
					break;
				case 3:
					GX_SetTexCopySrc(0, 0, 256, 256);
					GX_SetTexCopyDst(256, 256, GX_TF_RGBA8, GX_FALSE);
					GX_CopyTex(DofDefClr + 786432, GX_TRUE); 
					break;
				case 4:
					GX_SetTexCopySrc(0, 0, 512, 256);
					GX_SetTexCopyDst(512, 256, GX_TF_RGBA8, GX_FALSE);
					GX_CopyTex(DofDefClr, GX_TRUE); 
					break;
			}
		...
		if(frameadvance) {
			frame = (ASND_GetTickCounterVoice(0) / 1600) - 2;
			if(frame < 1) frame = 1;
			if(frame == framelastframe) frame++;
			framelastframe = frame;
			/*frame++;
			if(!dolphin || european) {
